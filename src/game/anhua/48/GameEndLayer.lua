...
function GameEndLayer:updateHeadImage(pBuffer)
    ...
    for key, var in pairs(GameCommon.player) do
        ...
        if pBuffer.wWinUser == key then --赢家
            ...
            self:updateImageInfo(allAvatar[index],var,pBuffer.lGameScore[var.wChairID+1],pBuffer.fWriteScoreArr[var.wChairID+1])
            index = index+1
        end
    end
end
...
function GameEndLayer:showMingTang(pBuffer)
    ...
    if Bit:_and(GameCommon.gameConfig.dwMingTang,0x10000) ~= 0 then 
        for i=1 ,#TY_cbCenterCard do
            for j=i+1 ,#TY_cbCenterCard do 
                if  (Bit:_and(TY_cbCenterCard[i],GameCommon.MASK_VALUE)) == (Bit:_and(TY_cbCenterCard[j] , GameCommon.MASK_VALUE)) then 
                    wTYCount = wTYCount +1
                end 
            end 
        end 
