...
--    self.meChairID = 0
    ...
    self.language = cc.UserDefault:getInstance():getIntegerForKey('volumeSelect', 0)  	--语言 0-普通话 1-方言
end

function GameCommon:getViewIDByChairID(wChairID, isInRoom)
    ...
    if isInRoom and wPlayerCount == 2 and viewID == 2 then
        viewID = 3
    end
...
--手牌资源 --small 获取手牌资源 大 和 小区分
function GameCommon:GetCardHand(data,isSmall)
    local imgCard = nil    
    ...
    if data == 0 then
        -- if cardIndex ~= 0 then
        --     imgCard = ccui.ImageView:create("huaihua/card_bg/card_bg1/card_bg_3.png")
        -- else
        --     imgCard = ccui.ImageView:create("huaihua/card_bg/card_bg0/card_bg_3.png")
        -- end
        imgCard = ccui.ImageView:create("anhua/card_bg/card_bg2/card_bg_3.png")
    elseif data==33 then
        -- if cardIndex == 0 then
    ...
    elseif data <= 10 then
        if isSmall then
            imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data,'phz_ss'))
        else
            imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data,'phz_s'))
        end
    elseif data > 10 then
        if isSmall then
            imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data-16,'phz_bs'))
        else
            imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data-16,'phz_b'))
        end
    else
        assert(false)
    end
...
function GameCommon:getSendOrOutCard(data, isSendCard)
    ...
    --     imgBg = ccui.ImageView:create(string.format("huaihua/card_bg/card_bg%d/card_send_card_bj.png", cardIndex))    
    -- else
    --     imgBg = ccui.ImageView:create(string.format("huaihua/card_bg/card_bg%d/card_out_card_bj.png", cardIndex))
    -- end
    imgBg = ccui.ImageView:create(string.format("anhua/card_bg/card_bg%d/card_out_card_bj.png", 2))
    ...
    elseif data==33 then
        ...
        -- end
    elseif data <= 10 then
        imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data,'phz_cp_s'))
    elseif data > 10 then
        imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data-16,'phz_cp_b'))
    else
        assert(false)
    end
...
function GameCommon:getDiscardCardAndWeaveItemArray(data)
    ...
    if data == 0 then
        -- if cardBgIndex ~= 0 then
        --     imgCard = ccui.ImageView:create("huaihua/card_bg/card_bg1/card_bg_2.png")
        -- else
        --     imgCard = ccui.ImageView:create("huaihua/card_bg/card_bg0/card_bg_2.png")
        -- end
        imgCard = ccui.ImageView:create("anhua/card_bg/card_bg2/card_bg_2.png")
    elseif data==33 then
        if cardIndex == 0 then
...
function GameCommon.cardPath( index,numb ,name)
    print('cxx---->>>>',string.format("anhua/card/card%d/%s%d.png",3,name,numb))
    return string.format("anhua/card/card%d/%s%d.png",3,name,numb)
end
...
function GameCommon:playAnimation(root, id, wChairID)
    ...
    --id chi  GameCommon.regionSound diyu
