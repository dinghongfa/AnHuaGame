...
local GameOperation = require("game.anhua.GameOperation")
...
local IsOpenTin = cc.UserDefault:getInstance():getBoolForKey('AnHuaisOpenTin', true)
...
function TableLayer:transferChairID(wChairID)
	local viewID = GameCommon:getViewIDByChairID(wChairID, true)
...
function TableLayer:showCountDown(wChairID,viewsID)
	self:resetUserCountTimeAni()
	local viewID = GameCommon:getViewIDByChairID(wChairID, true)
	if viewsID ~= nil then 
		viewID = viewsID
...
function TableLayer:showLeftCardCount(bLeftCardCount, bLeftCardData)
	...
	local uidipai = ccui.ImageView:create("anhua/ui/gameover/phz_s_dipai.png")
...
function TableLayer:updateLeftCardCount(bLeftCardCount, isEffects, isSendCardEffects)
	...
	if bLeftCardCount == 0 then
		path = 'anhua/ui/table/phz_comm_lastcard3.png'
	elseif bLeftCardCount >= 1 and bLeftCardCount <= 8 then
		path = 'anhua/ui/table/phz_comm_lastcard2.png'
	else
		path = 'anhua/ui/table/phz_comm_lastcard1.png'
	end
				...
				elseif var.cbWeaveKind == GameCommon.ACK_WEI then
					if GameCommon.weiCardType == 0 then
						...
						if GameCommon.tableConfig.nTableType == TableType_Playback or viewID == 1 or GameCommon.gameState == GameCommon.GameState_Over then
							card = GameCommon:getDiscardCardAndWeaveItemArray(v)
							--card:setColor(cc.c3b(150, 150, 150))
							local  card1 = GameCommon:getDiscardCardAndWeaveItemArray(0)
                            card:addChild(card1)
...
function TableLayer:showHandCard(wChairID, effectsType, isShowEndCard)
	...
	for key, var in pairs(cardStackInfo) do
		--胡息
		if IsOpenTin and not isShowEndCard and false then
...
function TableLayer:initUI()
	...
	Common:addTouchEventListener(ccui.Helper:seekWidgetByName(self.root, "Button_settings"), function()
		local path = self:requireClass('AHSetting')
	...
	local Image_roomicon = ccui.Helper:seekWidgetByName(self.root, "Image_roomicon")
	local Image_jsicon = ccui.Helper:seekWidgetByName(self.root, "Image_jsicon")
	...
	if GameCommon.tableConfig.nTableType > TableType_GoldRoom then
		...
		BitmapFontLabel_roomid:setString(GameCommon.tableConfig.wTbaleID)
		BitmapFontLabel_curju:setString(string.format("%d/%d", GameCommon.tableConfig.wCurrentNumber, GameCommon.tableConfig.wTableNumber))		
	elseif GameCommon.tableConfig.nTableType == TableType_GoldRoom then			
		...
		Image_roomicon:setVisible(false)
	...
	local callback = function()
		...
		Text_curtime:setString(time)
	end
	schedule(Text_curtime, callback, 1)
...
function TableLayer:setWaterMask()
	...
	mask:loadTexture('anhua/ui/water_mask/game_106.png')
end
...
function TableLayer:changeBgLayer()
	...
	if index >= 1 and index <= 4 then
		uiPanel_bg:loadTexture(string.format("anhua/bg/paohuzi_table_bg%d.jpg", index))
	end
end
...
function TableLayer:addVoice()
	...
	local function resetVoice()
		...
		local node = require("common.CircleLoadingBar"):create("anhua/ui/chat/btnMask.png")
...
function TableLayer:showPlayerInfo(infoTbl)       -- 查看玩家信息
	Common:palyButton()
	require("common.SceneMgr"):switchOperation(require("app.MyApp"):create(infoTbl, self):createGame("game.anhua.AHPersonInfoLayer"))
end
...
function TableLayer:playSpine(pBuffer)
    ...
    if not cusNode then
    	printInfo('global_node is nil for anhua')
    	return
    end
...
function TableLayer:EVENT_TYPE_SKIN_CHANGE(event)
	...
	IsOpenTin = cc.UserDefault:getInstance():getBoolForKey('AnHuaisOpenTin', true)
...
function TableLayer:setTPView(data)
	...
	for key, var in pairs(data.cbCardIndexTing) do
		if var > 0 then
			...
			local card = nil
			if key <= 10 then
				card = ccui.ImageView:create(string.format("anhua/card/listencard_%ds.png", key))
			else
				card = ccui.ImageView:create(string.format("anhua/card/listencard_%db.png", key - 10))
			end
...
function TableLayer:updatePlayerHuXi(wChairID)
    ...
    local uiText_huXi = ccui.Helper:seekWidgetByName(uiPanel_player,"Text_huXi")
    if viewID == 1 then
    	uiText_huXi:setString(string.format("%d",huXiCount + GameCommon.handHuXiNum) .. '胡息')
	else
		uiText_huXi:setString(string.format("%d",huXiCount) .. '胡息')
    end
end
