...
function GameEndLayer:onCreate(pBuffer)  
   ...
   -- local uiImage_continue = ccui.Helper:seekWidgetByName(self.root,"Image_continue")
    ...
    uiText_Gamename:setString("安化麻将") 
   ...
   -- uiImage_dice:setContentSize(cc.size(141.00+(line-1)*43,76.00))   
    ...
    for i = 1,GameCommon.gameConfig.bPlayerCount do
        ...
        -- end
        local uiText_mashu = ccui.Helper:seekWidgetByName(uiListView_mingTang,"Text_mashu")
        uiText_mashu:setString("中码"..string.format("%d",pBuffer.cbZhanNiaoCount[wChairID+1]))
        uiText_mashu:setColor(cc.c3b(181,137,81))
        ...
        uiText_hu:setColor(cc.c3b(106,179,72))
        ...
        --     --自摸
        if Bit:_and(pBuffer.wChiHuKind[i],0x0020) ~= 0 or Bit:_and(pBuffer.wChiHuKind[i],0x0040) ~= 0 then
            -- for j = 15, 20 do
                pBuffer.cbCardData[i][15] = pBuffer.cbChiHuCard[i][1]
            -- end
        end
        if pBuffer.wWinner[i] == true and  pBuffer.wProvideUser ~= wChairID  then          
            pBuffer.cbCardData[i][15] = pBuffer.cbChiHuCard[i][1]
        end 
        local cbCardData_Tem = {}
        for j = 1, #pBuffer.cbCardData[i] do
            if GameCommon.cbKingCard~=nil and( pBuffer.cbCardData[i][j]== GameCommon.cbKingCard[1] or pBuffer.cbCardData[i][j] == GameCommon.cbKingCard[2] )then
                table.insert(cbCardData_Tem,1,pBuffer.cbCardData[i][j])
            else
                table.insert(cbCardData_Tem,#cbCardData_Tem+1,pBuffer.cbCardData[i][j])
            end
        end
        pBuffer.cbCardData[i] = cbCardData_Tem
        ...
        for key, var in pairs(pBuffer.cbCardData[i]) do
            ...
            if data ~= 0 then
                ...
                if GameCommon.cbKingCard ~= nil and GameCommon.cbKingCard[1] ~= 0 then  
                    ...
                    for j = 1 , 2 do
                        if GameCommon.cbKingCard[j] ~= 0 and  GameCommon.cbKingCard[j] == data then 
                            card:addChild(liang)
                            liang:setPosition(43,64)
                            -- liang:setScale(cardScale)
                            --break
                        end 
                    end 
                end 
                ...
                if key > 14 and  pBuffer.cbChiHuCard[wChairID+1][1] ~= 0 and tableMark[1] == false then                    
                    tableMark[1] = true
                end 
                if key > 14 then
                ...
                elseif pBuffer.wProvideUser < GameCommon.gameConfig.bPlayerCount and pBuffer.wProvideUser == wChairID and pBuffer.wWinner[i] == true then
                    -- for i = 1, 2 do
                    if data == pBuffer.cbChiHuCard[wChairID+1][1] and tableMark[1] == false then
                        local a = true
                        for k = key+1 ,#pBuffer.cbCardData[i] do
                            if pBuffer.cbCardData[i][k] ~= nil and pBuffer.cbCardData[i][k] == pBuffer.cbChiHuCard[wChairID+1][1] then 
                                a = false
                            end 
                        end 
                        if a ==  true then 
...
function GameEndLayer:showMingTang(wChiHuKind,wSpecialKind)
    ...
    -- if Bit:_and(wChiHuKind,CHK_PING_HU) ~= 0 then
    --      desc =desc.."碰碰胡 "
    -- end    
    ...
    if Bit:_and(wChiHuKind,CHR_YING_ZHUANG) ~= 0 then
         desc =desc.."硬庄 "
    end
    ...
    if Bit:_and(wChiHuKind,CHR_GANG_SHANG_PAO) ~= 0 then
        desc =desc.."杠上炮"
    end
    if Bit:_and(wChiHuKind,CHR_QI_WANG) ~= 0 then
        desc =desc.."七王 "
    else
        if Bit:_and(wChiHuKind,CHR_LIU_WANG) ~= 0 then
            desc =desc.."六王 "
        else
            if Bit:_and(wChiHuKind,CHR_SI_WANG) ~= 0 then
                desc =desc.."四王 "
            else
                if Bit:_and(wChiHuKind,CHR_SAN_WANG) ~= 0 then
                    desc =desc.."三王 "
                end    
            end           
        end   
