...
function GameLayer:startGame(...)
    ...
    if language ~= nil and language == 0 then
        GameCommon.language = 0
    else
        GameCommon.language = 1
    end
    GameCommon.tingpai = cc.UserDefault:getInstance():getIntegerForKey('tingpaiSelect', 1) 
    self:resetPiaoFen()
...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_KING_CARD then                --王牌数据      
            _tagMsg.pBuffer.cbKingCard = {}                             --麻将列表
            for i = 1 , 2 do
                _tagMsg.pBuffer.cbKingCard[i] = luaFunc:readRecvByte()
            end	
        elseif subCmdID == NetMsgId.SUB_S_STARTHU_FOURKING then       --四王起手胡 
            _tagMsg.pBuffer.mStartHuUser = luaFunc:readRecvWORD()        --四王起手胡玩家
            _tagMsg.pBuffer.bUserTing = luaFunc:readRecvByte()        --13张起手四王   0:13张  1：14张     
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG then              --操作提示
            ...
            if (Bit:_and(_tagMsg.pBuffer.cbActionMask,GameCommon.WIK_CHI_HU) ~= 0) or (Bit:_and(_tagMsg.pBuffer.cbActionMask,GameCommon.WIK_BIHU) ~= 0) then           
                print("胡牌了+++++++++++++++++++++")
            end 
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_RESULT then              --操作结果
            ...
            if Bit:_and(_tagMsg.pBuffer.cbOperateCode,GameCommon.WIK_GANG) ~= 0 then
                print("+++++++++++++++杠+++++++++++++")
            end
...
function GameLayer:OnGameMessageRun(_tagMsg)
    ...
    if mainCmdID == NetMsgId.MDM_GR_USER then   
        if subCmdID == NetMsgId.SUB_GR_GAME_STATISTICS then
            self:removeAllChildren()
            -- local layer = require("game.majiang.GameMaJiangRoomEnd"):create(pBuffer)
            -- self:addChild(layer)
            local box = require("app.MyApp"):create(pBuffer):createGame("game.majiang.GameMaJiangRoomEnd")
            self:addChild(box)
        else
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_GAME_START_MAJIANG then
            ...
            GameCommon.cbKingCard = nil
            if  GameCommon.cbKingCard ~= nil  and GameCommon.cbKingCard[1] ~= 0 then
                self.tableLayer:doAction(NetMsgId.SUB_S_KING_CARD, pBuffer)
            end 
        ...
        elseif subCmdID == NetMsgId.SUB_S_KING_CARD then                --王牌数据    
            ...
            GameCommon.cbKingCard = pBuffer.cbKingCard
            if GameCommon.cbKingCard[1] == GameCommon.cbKingCard[2] then 
                GameCommon.cbKingCard[2] = 0
            end 
            self.tableLayer:doAction(NetMsgId.SUB_S_KING_CARD, pBuffer)
        elseif subCmdID == NetMsgId.SUB_S_STARTHU_FOURKING then                --王牌数据             
            self.tableLayer:doAction(NetMsgId.SUB_S_STARTHU_FOURKING, pBuffer)
        elseif subCmdID == NetMsgId.SUB_S_SpecialCard then
        ...
        elseif subCmdID == NetMsgId.SUB_S_JIA_PIAO_80 then
            ...
            self:runAction(cc.Sequence:create(cc.DelayTime:create(0),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG then              --操作提示
            if (Bit:_and(pBuffer.cbActionMask,GameCommon.WIK_CHI_HU) ~= 0) or (Bit:_and(pBuffer.cbActionMask,GameCommon.WIK_BIHU) ~= 0) then           
                print("胡牌了+++++++++++++++++++++")
            end 
        ...
        elseif subCmdID == NetMsgId.SUB_S_BAOTINGOUTCARD then              --报听可删牌数据      
            ...
            self.tableLayer:BaoTingCardData({cbBTCard = pBuffer.cbBTCard,mBTHuCard = pBuffer.mBTHuCard})
       elseif subCmdID == NetMsgId.SUB_S_ALONE_BAOTINGCARD then           --报听可胡哪些牌数据   
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_RESULT then              --操作结果
            ...
            if Bit:_and(pBuffer.cbOperateCode,GameCommon.WIK_GANG) ~= 0 then
                print("+++++++++++++++杠+++++++++++++")
            end
        elseif subCmdID == NetMsgId.SUB_S_CASTDICE_RESULT then             --要帅结果
            GameCommon.mINGang[pBuffer.wCurrentUser] = true
            self.tableLayer:doAction(NetMsgId.SUB_S_CASTDICE_RESULT_92, pBuffer)--
            self:updatehandplate()
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束
          ...
          --  self:updatePlayerlScore()
            ...
            self.tableLayer:doAction(NetMsgId.SUB_S_GAME_END_MAJIANG,{wWinner = pBuffer.wWinner,wProvideUser = pBuffer.wProvideUser,wChiHuKind = pBuffer.wChiHuKind})
            ...
            for i = 1, GameCommon.gameConfig.bPlayerCount do
                ...
                local a = GameCommon.player[i-1].cbCardCount     
                local cbCardData_Tem = {}
                for j = 1, GameCommon.player[i-1].cbCardCount do
                    if ( GameCommon.cbKingCard~=nil and GameCommon.player[i-1].cbCardData[j] == GameCommon.cbKingCard[1] or GameCommon.player[i-1].cbCardData[j] == GameCommon.cbKingCard[2]) then
                        -- local num = cbCardData[i]
                        -- table.remove(cbCardData,i)
                        table.insert(cbCardData_Tem,1,GameCommon.player[i-1].cbCardData[j])
                    else
                        table.insert(cbCardData_Tem,#cbCardData_Tem+1,GameCommon.player[i-1].cbCardData[j])
                    end
                end
                GameCommon.player[i-1].cbCardData = cbCardData_Tem
                local a = 1
          -- GameCommon.player[i-1].cbCardCount
                self.tableLayer:showHandCard({wChairID = i-1,GameEnd = true})
            end
            ...
            uiPanel_end:runAction(cc.Sequence:create(
                ...
                cc.CallFunc:create(function(sender,event) 
                    uiPanel_end:addChild(require("game.majiang.92.GameEndLayer"):create(pBuffer))
                end),
...
--                            GameCommon:ContinueGame(GameCommon.tableConfig.cbLevel)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then
            ...
            if GameCommon.cbKingCard[1] ~= 0 then 
                local uiImage_WP = ccui.Helper:seekWidgetByName(self.root,"Image_WP")
            ...
            for i = 1, GameCommon.gameConfig.bPlayerCount do
                ...
                local INGang = false
                for j = 1 , pBuffer.cbWeaveCount[i] do 
                    if (Bit:_and(pBuffer.WeaveItemArray[i][j].cbWeaveKind,GameCommon.WIK_GANG) ~= 0) then
                        if pBuffer.wDiceCard[1] == 0 then 
                            GameCommon.mINGang[i-1] = true
                        else
                            INGang = true
                        end 
                    end 
                end 
            --    pBuffer.WeaveItemArray[i]    --    WeaveItemArray[i][j].cbWeaveKind
	    ...
	    -------------------
            ...
            if pBuffer.wDiceCardOne ~= 0 or pBuffer.wDiceCard[1] ~= 0  then
                self.tableLayer:doAction(NetMsgId.SUB_S_CASTDICE_RESULT_92, {wCurrentUser = pBuffer.wCurrentUser, wDiceCount = 0,wDiceCard= pBuffer.wDiceCard})
...
function GameLayer:updatePlayerlfatigue()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        local uiText_score = ccui.Helper:seekWidgetByName(uiPanel_player,"Text_score")
        local dwGold = GameCommon.tableConfig.lUserScore[i]
