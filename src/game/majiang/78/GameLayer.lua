...
function GameLayer:startGame(...)
    ...
    GameCommon.mHuCard = {}
    GameCommon.cdTingData = nil   -- 报停牌位置
...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
       ...
       if subCmdID == NetMsgId.SUB_GR_USER_READY then
        ...
        elseif subCmdID == NetMsgId.SUB_GR_USER_LEAVE then
            ...
            if GameCommon.dwUserID == dwUserID then
                ...
                EventMgr:dispatch(EventType.USER_LEAVETABLE)
            end
            return true
        ...
        elseif subCmdID == NetMsgId.SUB_GR_USER_COME then 
            ...
            data.cbDiscardCard = {}
            data.gangFen = data.lScore --做杠分临时显示
            data.cardNode = {}
            printInfo(data)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_JIA_PIAO then
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束
            ...
            _tagMsg.pBuffer.bQiangGangTF ={}
            for i = 1 , 4 do
                _tagMsg.pBuffer.bQiangGangTF[i] = luaFunc:readRecvBool()    --抢杠胡     
            end
            _tagMsg.pBuffer.m_GongGangCard = luaFunc:readRecvByte()   --抢杠胡的牌
            _tagMsg.pBuffer.cbZhaNiaoWOWO = luaFunc:readRecvByte()
            _tagMsg.pBuffer.cbAddGangScore = {}
            for i = 1, 4 do
                _tagMsg.pBuffer.cbAddGangScore[i] = luaFunc:readRecvLong()
            end
            _tagMsg.pBuffer.cbAddHu = {}
            for i = 1, 4 do
                _tagMsg.pBuffer.cbAddHu[i] = luaFunc:readRecvLong()
            end

            _tagMsg.pBuffer.mYMQZNiaoCOUNT = {}             --纯胡分<中马分>
            for i = 1, 4 do
                _tagMsg.pBuffer.mYMQZNiaoCOUNT[i] = luaFunc:readRecvByte()
            end
...
function GameLayer:OnGameMessageRun(_tagMsg)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_GAME_START_MAJIANG then
            ...
            if GameCommon.gameConfig.mLaiZiCount == 1 then
                cbLeftCardCount = cbLeftCardCount + 4
            elseif GameCommon.gameConfig.mLaiZiCount == 2 then
                cbLeftCardCount = cbLeftCardCount + 8
            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_RESULT then              --操作结果
            ...
            if Bit:_and(pBuffer.cbOperateCode,GameCommon.WIK_GANG) ~= 0 then
                for i = 1, GameCommon.gameConfig.bPlayerCount do
                    local wChairID = i-1
                    GameCommon.player[wChairID].gangFen = GameCommon.player[wChairID].gangFen + pBuffer.lGameScore[i]
                end
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束
            ...
            self.tableLayer:endAction(pBuffer)  
            ...
            local _, delayTimer = self.tableLayer:getZNNumOrTime(pBuffer.bZhaNiao)
            uiPanel_end:runAction(cc.Sequence:create(
            cc.DelayTime:create(delayTimer),
            cc.CallFunc:create(function(sender,event)          
                uiPanel_end:addChild(require("game.majiang.78.GameEndLayer"):create(pBuffer))                         
            end),
            cc.DelayTime:create(17),
            cc.CallFunc:create(function(sender,event) 
        ...
        if subCmdID == NetMsgId.SUB_GF_SCENE then
           ...
           -- self:updatePlayerlScore()
            ...
            if pBuffer.wCurrentUser == 65535 and pBuffer.wOutCardUser < GameCommon.gameConfig.bPlayerCount and pBuffer.cbOutCardData ~= 0 then
                ...
                self.tableLayer:doAction(NetMsgId.SUB_S_OUT_CARD_RESULT, {wOutCardUser = pBuffer.wOutCardUser, 
                    cbOutCardData = pBuffer.cbOutCardData, isNoDelete = true, wLastOutCardUser = pBuffer.wLastOutCardUser})
            else
                self.tableLayer:doAction(NetMsgId.SUB_S_OUT_CARD_NOTIFY_MAJIANG, {wOutCardUser = pBuffer.wCurrentUser, 
                    wLastOutCardUser = pBuffer.wLastOutCardUser})
            end
            if pBuffer.cbActionMask ~= 0 then
                if pBuffer.wOutCardUser == GameCommon:getRoleChairID() or pBuffer.wOutCardUser == 65535 then
                    self.tableLayer:doAction(NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG,{wResumeUser = 0,cbActionMask = pBuffer.cbActionMask, 
                        cbActionCard = pBuffer.cbActionCard,bIsSelf = true,cbGangCard = pBuffer.cbGangCard, wLastOutCardUser = pBuffer.wLastOutCardUser})
                else
                    self.tableLayer:doAction(NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG,{wResumeUser = 0,cbActionMask = pBuffer.cbActionMask, 
                        cbActionCard = pBuffer.cbActionCard,bIsSelf = false,cbGangCard = pBuffer.cbGangCard,wLastOutCardUser = pBuffer.wLastOutCardUser})
                end
            end
...
function GameLayer:updatePlayerInfo()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        if GameCommon.player == nil or GameCommon.player[wChairID] == nil then
            ...
            -- --uiText_score:setString(string.format("%d",GameCommon.player[wChairID].lScore)) 
...
function GameLayer:updatePlayerlScore()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_player%d",viewID))
        local uiText_score = ccui.Helper:seekWidgetByName(uiPanel_player,"Text_score")
        local dwGold = Common:itemNumberToString(GameCommon.player[wChairID].gangFen)
        if uiText_score then
            uiText_score:setString(tostring(dwGold))
        end
    end
end
...
function GameLayer:playSZAnim( func,pBuffer )
    ...
    armature:runAction(cc.Sequence:create(cc.DelayTime:create(1),cc.CallFunc:create(function(sender,event) 
        ...
        tableSiceCount[1] = pBuffer.mShaiZi[1]
        tableSiceCount[2] = pBuffer.mShaiZi[2]
        print ("+++++++骰子大小22222+++++++",tableSiceCount[1],tableSiceCount[2])
...
-- 重置杠分 @chg
function GameLayer:resetGanScore(  )
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        local wChairID = i - 1
        if GameCommon.player == nil or GameCommon.player[wChairID] == nil then

        else
            GameCommon.player[wChairID].gangFen = GameCommon.player[wChairID].lScore
        end
    end
