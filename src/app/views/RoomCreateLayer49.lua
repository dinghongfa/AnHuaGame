...
function RoomCreateLayer:onCreate(parameter)
    ...
    local csb = cc.CSLoader:createNode("RoomCreateLayer49.csb")
    self:addChild(csb)
    ...
    if self.recordCreateParameter["dwMingTang"] ~= nil and Bit:_and(0x0C00,self.recordCreateParameter["dwMingTang"]) ~= 0 then
        self:setDefaultCheckBox(items[2])
    end
    if self.recordCreateParameter["STWK"] ~= nil and self.recordCreateParameter["STWK"] == 1 then    --  三提五坎
        self:setDefaultCheckBox(items[3])
    end
...
function RoomCreateLayer:onEventCreate(nTableType)
    ...
    tableParameter.dwMingTang = Bit:_xor(tableParameter.dwMingTang,0x0C00)  
    if items[2]:isBright() then
        tableParameter.dwMingTang = Bit:_or(tableParameter.dwMingTang,0x0C00)
    end
    tableParameter.STWK = 0
    if items[3]:isBright() then
