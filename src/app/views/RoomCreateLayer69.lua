...
function RoomCreateLayer:onCreate(parameter)
    ...
    local csb = cc.CSLoader:createNode("RoomCreateLayer69.csb")
    self:addChild(csb)
    ...
    local uiPanel_renShu_1 = ccui.Helper:seekWidgetByName(self.root,"Panel_renShu_1")
    --选择人数
    ...
    Common:addCheckTouchEventListener(items)
    if self.recordCreateParameter["bPaPo"] == 0 then
        self:setDefaultCheckBox(items[4])
    elseif self.recordCreateParameter["bPaPo"] == 2 then
        self:setDefaultCheckBox(items[3])
    elseif self.recordCreateParameter["bPaPo"] == 3 then
        self:setDefaultCheckBox(items[2])
    else
        self:setDefaultCheckBox(items[1])
    end
    ...
    if self.recordCreateParameter["bStartTun"] == 1 then
        self:setDefaultCheckBox(items[1])
    end

    if self.recordCreateParameter["bStartBanker"] == 1 or self.recordCreateParameter["bStartBanker"] == nil then
        self:setDefaultCheckBox(items[2])
    end
    ...
    elseif self.recordCreateParameter["bCanHuXi"] == 21 then
        self:setDefaultCheckBox(items[3])
    elseif self.recordCreateParameter["bCanHuXi"] == 18 then
        self:setDefaultCheckBox(items[2])
    else
        self:setDefaultCheckBox(items[1])
    end
...
function RoomCreateLayer:onEventCreate(nTableType)
    ...
    print('enter onEventCrete nTableType...')
    ...
    local items = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(2),"ListView_parameter"):getItems()
    if items[1]:isBright() then
         tableParameter.bPaPo = 1
    elseif items[2]:isBright() then
        tableParameter.bPaPo = 3
    elseif items[3]:isBright() then
        tableParameter.bPaPo = 2
    elseif items[4]:isBright() then
        tableParameter.bPaPo = 0
    else
        return
    end
   ...
   if self.showType ~= 2 and (nTableType == TableType_FriendRoom or nTableType == TableType_HelpRoom) then
    ...
    dump(tableParameter,"cxx::==")
end

function RoomCreateLayer:setDefaultCheckBox(item)
