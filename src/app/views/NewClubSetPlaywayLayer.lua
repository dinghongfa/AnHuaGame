...
function NewClubSetPlaywayLayer:initPlaywayItem(item, count, index)
    ...
    Text_cusName:setColor(cc.c3b(255, 0, 0))
    ...
    if self.allTableData then
	        ...
	        for _,v in pairs(self.allTableData) do
	            if v and v.wTableSubType == playwayid then
	                if (kindid == 51 or kindid == 53 or kindid == 55 or kindid == 56 or kindid == 57 or kindid == 58 or kindid == 59) and v.tableParameter.bCanPlayingJoin == 1 and v.wCurrentChairCount < v.wChairCount  then
	                    require("common.SceneMgr"):switchTips(require("app.MyApp"):create(v.dwTableID,self:getEnterTableFigueValue(index)):createView("InterfaceJoinRoomNode"))
	                    return
	                elseif v.bIsGameStart == false and v.wCurrentChairCount < v.wChairCount then
	                    require("common.SceneMgr"):switchTips(require("app.MyApp"):create(v.dwTableID,self:getEnterTableFigueValue(index)):createView("InterfaceJoinRoomNode"))
	                    return
	                end
	            end
	    ...
	    local tableNum = 0
	    local playwayid = self.clubData.dwPlayID[index]
	    for i,v in pairs(self.allTableData) do
	        if v and v.dwTableID and v.wTableSubType == playwayid then
	            tableNum = tableNum + 1
	        end
	    end
    ...
    local parameter = self.clubData.tableParameter[index]
    Text_people:setString(string.format('%d人', parameter.bPlayerCount))
    local jushu = self.clubData.wGameCount[index]
    Text_jushu:setString(string.format('%d局', jushu))
